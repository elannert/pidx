...
from    (
        Select pmonth, user_id, 
        		case when breadth is Null then 
        				...
        				over (partition by user_id order by pmonth asc) 
        		else breadth 
        		end as breadth,
        		...
        		case when duration is Null then 
        				lag(duration) ignore nulls 
        				over (partition by user_id order by pmonth asc) 
        		else duration 
        		end as duration,
        		case when density is Null then 
        				lag(density) ignore nulls 
        				over (partition by user_id order by pmonth asc) 
        		else density 
        		end as density
        from    (		
                Select	users_dates.pmonth, users_dates.user_id, breadth, depth, duration, density 
                from	(
